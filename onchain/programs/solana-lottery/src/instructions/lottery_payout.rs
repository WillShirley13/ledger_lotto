...
pub struct LotteryPayout<'info> {
    ...
    pub first_winner: SystemAccount<'info>,
    ...
    pub second_winner: SystemAccount<'info>,
    ...
    pub third_winner: SystemAccount<'info>,
...
pub fn process_lottery_payout(ctx: Context<LotteryPayout>) -> Result<()> {
    ...
    let first_winner: &mut AccountInfo<'_> = &mut ctx.accounts.first_winner.to_account_info();
    let second_winner: &mut AccountInfo<'_> = &mut ctx.accounts.second_winner.to_account_info();
    let third_winner: &mut AccountInfo<'_> = &mut ctx.accounts.third_winner.to_account_info();
    ...
    if lottery_vault.latest_loto_winners.first_place.is_some() {
        transfer_lamports(
        ...
        msg!("First place reward transferred to {}", first_winner.key);
    } else {
        msg!("First place winner not found");
    }

    // Second place winner payout
    if lottery_vault.latest_loto_winners.second_place.is_some() {
        transfer_lamports(
            &lottery_vault.to_account_info(),
            second_winner,
            second_place_reward,
        ...
        msg!("Second place reward transferred to {}", second_winner.key);
    } else {
        msg!("Second place winner not found");
    }

    // Third place winner payout
    if lottery_vault.latest_loto_winners.third_place.is_some() {
        transfer_lamports(
            &lottery_vault.to_account_info(),
            third_winner,
            third_place_reward,
        ...
        msg!("Third place reward transferred to {}", third_winner.key);
    } else {
